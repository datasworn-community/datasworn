...
import type { DataswornSource } from '@datasworn-community/core'
...
export async function writeJSON(
	...
	const allPaths = Array.isArray(filePath) ? filePath : [filePath]
	// dedupe identical destinations (callers may pass the same path twice when
	// two conceptually distinct outputs happen to coincide); otherwise the file
	// gets copied onto itself, which can race with its own write and truncate it
	const seen = new Set<string>()
	const pathParams = allPaths.filter((destination) => {
		const key = typeof destination === 'string' ? destination : destination.name
